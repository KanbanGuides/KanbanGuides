...
Every existing PDF is supplied and preserved. A PDF is generated only when an edition's `pdf.yaml` lists it under `downloads` with `handling: generated`. Cover credits come from `site/data/contributions/<guide>.yml` (role `creator` = authors) and `<guide>.<lang>.yml` (translation teams). Do not put fonts, `dir`, `author` or `translators` in guide front matter.
